...
class WatchlistServiceTests(TestCase):
    def test_generate_watchlist_keeps_current_day_locked_pool_after_trading_hours_when_sources_fail(self) -> None:
        ...
        cached = locked_pool(30)

        with (
            patch.object(watchlist_service, "get_settings", return_value={"quote_source": "自动切换", "auto_pool_size": 30}),
            patch.object(watchlist_service, "fetch_auto_stock_pool", return_value=failed_pool),
            ...
            patch.object(watchlist_service, "_watchlist_api", return_value=[{"code": "600000"}] * 30),
        ...
        self.assertIn("已保留今日已锁定初筛池 30 只", result["message"])
