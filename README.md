...
TZ 是与同花顺配合使用的个人交易纪律、账户记录和复盘工作台。默认内置“五日线回踩”模式，保留“模式2”规则框架，并已启用“十日线缩量回踩隔日反弹”模式三。它不做全市场选股、不提供实时荐股、不连接券商，也不自动下单。

同花顺负责行情、股票选择和真实下单；TZ 负责：

- 模拟训练与实盘记录两套完全独立的账户账本；
- 五日线回踩、模式2、十日线缩量回踩隔日反弹 三套交易模式视图；
- 人工登记买入、卖出和历史成交；
- 账户级现金、总资产、总持仓与当前交易模式规则校验；
...
账户资金以 `mode` 为强制分区键，交易方法相关数据以 `mode + strategy_id` 为强制分区键。
...
- `mode3`：十日线缩量回踩隔日反弹，尾盘缩量阴线回踩十日线买入，次日早盘纪律退出。

账户本金、现金、总资产、账户总持仓、手续费、行情和对账设置按模拟/实盘账户保存。交易流水、规则审计、当前模式持仓监控、延后决定、备注、通知和复盘按当前交易模式过滤展示，模式之间不会互相污染。
